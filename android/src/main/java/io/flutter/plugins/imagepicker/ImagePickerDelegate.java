...
public class ImagePickerDelegate
  ...
  private void handleCaptureImageResult(int resultCode) {
    if (resultCode == Activity.RESULT_OK) {
      Uri uri=pendingCameraMediaUri != null
              ? pendingCameraMediaUri
              : Uri.parse(cache.retrievePendingCameraMediaUriPath());
      handleImageResult(uri.getPath(), true);
//      final Date dateTimeCreatedAt =new Date();
...
//              Date now =new Date();
//              long diff =  now.getTime() - dateTimeCreatedAt.getTime();
//              Log.d("handleCaptureImageResult","miliseconds "+diff);
